...
aut <- fread('C:\\Users\\Ahmad\\Desktop\\MSDS\\MSDS660\\week 7\\in class\\autism_medication.csv')
View(aut)
...
# p-value = 0.06957 We accept the null hypothesis and state that the medians are NOT significantly different.


#Mann-Whitney U test
# two-sided test (Is the new drug greater or less effective than the placebo?)
wilcox.test(aut$Before.Treatment, aut$After.Treatment)
# p-value = 0.1.391 not quite significant so we fail to reject the null hypnosis and 
...
alb <- fread('C:\\Users\\Ahmad\\Desktop\\MSDS\\MSDS660\\week 7\\in class\\albumin.csv')
View(alb)
str(alb)
...
ks.test(aut$Before.Treatment, aut$After.Treatment, alternative = 'greater')
...
# We accept the null hythpothesis and there is NO relationship between variables.
